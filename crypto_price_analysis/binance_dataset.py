...
class BinanceDataset:
    ...
    def __init__(self, directory: Path, ticker: str, interval: str, max_gap_size: int):
        ...
        match ticker:
            ...
            case 'XMRUSDT': 
                self.default_date = date(2019, 3, 15)
            case 'XRPUSDT':
    ...
    def download_csv_file(self, date_str: str, monthly: bool) -> bool:
        ...
        zip_data = requests.get(csv_file_url, timeout=10)
        provided_checksum = requests.get(checksum_file_url, timeout=10).text[:64]
    ...
    def download_binance_dataset(self):
        """
        creates a new directory inside self.directory named from the ticker and interval
        """
        ...
        while d != end_date:
            ...
            if d.day == 1 and (d.year != end_date.year or d.month != end_date.month):
                ...
                if csv_file_path_month.exists():
                    data_list.append(pd.read_csv(csv_file_path_month, header=None, index_col=0, usecols=[0,1,2,3,4,5,8,9], names=columns, dtype=np.float64))
                    d = incr_month(d)
                    continue

            if csv_file_path_day.exists():
                data_list.append((pd.read_csv(csv_file_path_day, header=None, index_col=0, usecols=[0,1,2,3,4,5,8,9], names=columns, dtype=np.float64)))
    ...
    def preprocess_binance_dataset(self):
        """
        filters the dataset previously downloaded
        """
        ...
        data = data.asfreq(pd.Timedelta(self.interval)).interpolate('linear', limit=self.max_gap_size)
